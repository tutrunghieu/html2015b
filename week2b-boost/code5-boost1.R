...
h1("-------------plotting data");
dev.new(); print( qplot(Petal.Width, Sepal.Width, colour=Species, data=A) );
dev.new(); print( qplot(Petal.Width, Petal.Length, colour=Species, data=A) );


h1("-------------model fitting");
M <- train(Species ~ ., method="gbm", data=A, verbose=TRUE);
# cat("\n"); print(M$finalModel)
